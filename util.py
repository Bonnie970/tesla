...
def load_path_adobe(alphas, trimaps, FGs, BGs, RGBs, loss='total_loss'):
    # loss: 'total_loss': requires alphas, trimaps, FGs, BGs, RGBs
    #       'alpha_loss': requires alphas, trimaps, RGBs
    #       'no_loss': requires trimaps, RGBs
    '''
        rgb:
            0001.png
            ...
    ...
    RGBs_abspath = []
    if loss == 'no_loss':         
        image_names = os.listdir(trimaps)
...
def load_single_image_adobe(alpha_path, FG_path, BG_path, RGB_path):
	...
	rgb = misc.imread(RGB_path)
        ...
        # this returns 4 chanels sometime
	fg = misc.imread(FG_path, mode='RGB')
	...
	if alpha.shape[0] != image_height:
        ...
        #print alpha.shape, trimap.shape, fg.shape, bg.shape, rgb.shape
	batch_i = batch_i.astype(np.float32)
	return batch_i
...
def load_data_adobe(batch_alpha_paths=None,
                    batch_FG_paths=None,
                    batch_BG_paths=None,
                    batch_RGB_paths=None, 
                    loss = 'total_loss',
                    batch_trimap_paths=None):
    # loss: 'no_loss': requires trimap_path, RGB_path
    #       'alpha_loss': requires trimap_path, RGB_path, alpha_path
    #       'total_loss': requires trimap_path, RGB_path, alpha_path, FG_path, BG_path
    if loss == 'total_loss':
        batch_size = batch_alpha_paths.shape[0]
        train_batch = Parallel(n_jobs=4, verbose=1, backend="threading")(delayed(load_single_image_adobe)(batch_alpha_paths[i], batch_FG_paths[i], batch_BG_paths[i], batch_RGB_paths[i]) for i in range(batch_size))
    elif loss == 'no_loss': 
        batch_size = batch_trimap_paths.shape[0]
        train_batch = Parallel(n_jobs=4, verbose=1, backend="threading")(delayed(load_single_image_predict)(batch_trimap_paths[i], batch_RGB_paths[i]) for i in range(batch_size))
    elif loss == 'alpha_loss':
        batch_size = batch_trimap_paths.shape[0]
        train_batch = Parallel(n_jobs=4, verbose=1, backend="threading")(delayed(load_single_image_predict)(batch_trimap_paths[i], batch_RGB_paths[i],batch_alpha_paths[i]) for i in range(batch_size))
    
    train_batch = np.stack(train_batch)
    return train_batch

def load_data_adobe_nocrop(batch_alpha_paths,
                           batch_FG_paths,
                           batch_BG_paths,
                           batch_RGB_paths):
	...
	batch_size = batch_alpha_paths.shape[0]
        ...
        for i in range(batch_size):
            ...
            if alpha.shape[0] != image_height:
                ...
                fg = misc.imresize(fg, (image_height,image_width))
                bg = misc.imresize(bg, (image_height,image_width))
            ibatch = np.concatenate([alpha, trimap, rgb - g_mean, fg, bg, rgb],2)
            ibatch = ibatch.astype(np.float32)
            train_batch.append(ibatch)
	train_batch = np.stack(train_batch)
        
        return train_batch

def load_data_adobe_processed(batch_alpha_paths, batch_trimap_paths,
                           batch_FG_paths,
                           batch_BG_paths,
                           batch_RGB_paths):
	...
	batch_size = batch_alpha_paths.shape[0]
        train_batch = []
        for i in range(batch_size):
            alpha = np.expand_dims(misc.imread(batch_alpha_paths[i],'L'),axis=2)
            trimap = np.expand_dims(misc.imread(batch_trimap_paths[i],'L'),axis=2)
            rgb = misc.imread(batch_RGB_paths[i])
            fg = misc.imread(batch_FG_paths[i])
            bg = misc.imread(batch_BG_paths[i])
