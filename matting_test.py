...
def main(_):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options = gpu_options)) as sess:
        ...
        sess.graph.finalize()
        #trimap_clip = os.listdir(dataset_trimap)
        for trimap_clip in [1]:#trimap_clips:
            #clip_objects = os.listdir(os.path.join(dataset_trimap,trimap_clip))
            for clip_object in [1]:#clip_objects:
                #trimap_images = os.listdir(os.path.join(dataset_trimap, trimap_clip, clip_object))
                for trimap_image in os.listdir(dataset_trimap):#trimap_images:
                    #trimap_path = os.path.join(dataset_trimap, trimap_clip, clip_object, trimap_image)
                    trimap_path = os.path.join(dataset_trimap, trimap_image)
                    RGB_path = os.path.join(dataset_RGB, trimap_image)
                    ...
                    save_path = os.path.join(FLAGS.pred_path, trimap_image)
                    if not os.path.exists(os.path.join(FLAGS.pred_path)):
                        os.makedirs(os.path.join(FLAGS.pred_path))
